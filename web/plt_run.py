...
def process_requests(current_health, requests, user_to_server_mapping, content_sizes, content_importance):
    """
    根据请求序列处理用户请求，模拟服务器博弈过程。

    参数：
    current_health: list[float], 当前20台服务器的健康评分。
    requests: list[tuple], 用户请求列表，每个元组为(user_id, content_id)。
    user_to_server_mapping: dict[int, int], 用户到服务器的映射。
    content_sizes: dict[int, float], 内容大小。
    content_importance: dict[int, float], 内容重要性。

    返回：
    dict: 每台服务器的状态，包括内容和剩余容量。
    """
    servers = {i: {"contents": {}, "remaining_capacity": 1000, "prob_cache": 0.5} for i in range(20)}
    ...
    for idx, (user_id, content_id) in enumerate(requests):
        ...
        direct_requests[server_id] += 1
        content_size = content_sizes.get(content_id, 1)  # 默认内容大小为1
        content_import = content_importance.get(content_id, 1)  # 默认重要性为1
        # 更新每个服务器的当前综合加权稳定性， 平均到统计数组里面
        data_weighted_stability[server_id] += current_health[server_id] * content_import / total_importance
        # 更新全局综合加权稳定性
        global_data_weighted_stability.append(sum(data_weighted_stability) / len(data_weighted_stability))
        if content_id in servers[server_id]["contents"]:
            ...
            direct_hit[server_id] += 1
        else:
            
            print(f"Request handled: User {user_id} requested Content {content_id} -> Not in Server {server_id}, starting game...")

            # 初始化收益矩阵
            discount_factor = 0.5  # 可动态调整
            matrix = payoff_matrix(content_import, discount_factor)

            # 初始化策略概率
            initX = random.uniform(0, 1)
            initY = random.uniform(0, 1)

            # 计算演化路径
            epoch = 100
            dt = 0.01
            x_path, y_path = calculateValue(initX, initY, dt, epoch, matrix)
            trajectories.append((x_path, y_path))
            # 最终策略概率决定缓存操作
            final_x = x_path[-1]
            final_y = y_path[-1]
            ...
            if final_x > final_y:
                ...
                if servers[server_id]["remaining_capacity"] >= content_size:
                    ...
                    to_remove = min(servers[server_id]["contents"], key=servers[server_id]["contents"].get, default=None)
                    if to_remove:
                        servers[server_id]["remaining_capacity"] += content_sizes[to_remove]
                        del servers[server_id]["contents"][to_remove]
                    servers[server_id]["contents"][content_id] = content_import
                    servers[server_id]["remaining_capacity"] -= content_size
                print(f"Content {content_id} cached on Server {server_id}")
            else:
                # 玩家 B（其他服务器）缓存内容
                target_server = random.choice([s for s in range(20) if s != server_id])
                if servers[target_server]["remaining_capacity"] >= content_size:
                    servers[target_server]["contents"][content_id] = content_import
                    servers[target_server]["remaining_capacity"] -= content_size
                else:
                    to_remove = min(servers[target_server]["contents"], key=servers[target_server]["contents"].get, default=None)
                    if to_remove:
                        servers[target_server]["remaining_capacity"] += content_sizes[to_remove]
                        del servers[target_server]["contents"][to_remove]
                    servers[target_server]["contents"][content_id] = content_import
                    servers[target_server]["remaining_capacity"] -= content_size
                print(f"Content {content_id} cached on Server {target_server}")
