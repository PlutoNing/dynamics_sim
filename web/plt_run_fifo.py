...
def process_requests_fifo(current_health, requests, user_to_server_mapping, content_sizes, content_importance):
    """
    返回：
    dict: 每台服务器的状态，包括内容和剩余容量。
    """
    servers = {i: {"contents": {}, "remaining_capacity": 1000, "insert_order": deque()} for i in range(20)}
    ...
    for idx, (user_id, content_id) in enumerate(requests):
        ...
        if content_id in servers[server_id]["contents"]:
            ...
            if servers[server_id]["remaining_capacity"] >= content_size:
                ...
                servers[server_id]["insert_order"].append(content_id)
                servers[server_id]["remaining_capacity"] -= content_size
            else:
                # FIFO 替换策略
                to_remove = servers[server_id]["insert_order"].popleft()
